...
| ballista.shuffle.reader.max_local_readers                | UInt64  | 4         | Number of node-local shuffle blocks a reduce task reads and decodes concurrently, on the blocking pool. Each reader may run up to 2 batches ahead of the consumer, so peak buffered batches per task is roughly 3x this value. 1 restores the previous fully-serial local read.                                                                                                                                                                                                                                               |
