...
| ballista.shuffle.reader.max_local_readers                | UInt64  | the executor's vcores | Number of node-local shuffle blocks a reduce task reads and decodes concurrently, on the blocking pool. Defaults to the executor's vcores, which the executor stamps in when it builds a task's session config; this static default applies to standalone and in-process use. Each reader holds at most 2 batches, so peak buffered batches per task is roughly twice this value. 1 restores the previous fully-serial local read.                                                                                            |
